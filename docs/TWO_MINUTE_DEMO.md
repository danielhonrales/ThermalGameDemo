...
5. **WATCH OUT FOR LASERS** appears before the first head-height sweep at 10 seconds. One laser sweeps until 35 seconds, then stops while the arena changes. A perpendicular laser starts at 44 seconds. A dim amber line previews each laser for two seconds at its entry edge; a low hum grows as it approaches. The status below the clock shows the current phase. Duck under the beam. Laser damage bypasses the hand shield.
...
The first 20 seconds have half damage so first-time participants can learn the poses. Health is 300 HP; beam and laser hits are 20 damage before the early reduction and temporary invulnerability. The heal appears at 20 seconds. At 35 seconds, an amber **SUDDEN DEATH** countdown announces the new arena. Drones move the cover from 36 seconds, and the new layout is stable before 40 seconds. At 40 seconds the floor changes and the final 20-second sudden-death phase begins. The scene uses fewer sparks and less full-view red. Your corner health bar changes from green through yellow to red; the opponent's red health bar appears above them.
