...
1. Download `ThermalGameDemo-LAN-auto-install-kit.zip` from the [prototype GitHub release](https://github.com/danielhonrales/ThermalGameDemo/releases/tag/quest-lan-auto-2026-09-23) and unzip it. From the unzipped folder, use the commands in [SETUP.md](SETUP.md), starting with **Prepare the router and devices**. Skip **Clone and build the Unity project**; the APK is already in the bundle as `ThermalGameDemo-Quest-LAN.apk`.
...
3. Both headsets use the same APK and choose host/client automatically. Push the bundled Pi output config to Quest A. For Quest B, create its output config with its own Pi address and `quest-b` label. Restart each app after pushing output configs.
